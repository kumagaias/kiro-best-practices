...
**IMPORTANT - Secret Scanning with Gitleaks**:
- Gitleaks is automatically run before every commit via Kiro hook
- Install gitleaks: `brew install gitleaks` (macOS) or see [gitleaks installation](https://github.com/gitleaks/gitleaks#installing)
- Commit is blocked if secrets are detected
- Use `.gitleaksignore` to suppress false positives (sparingly)
- For Git-based workflows, also add pre-push hook:
```bash
...
**IMPORTANT - Pre-push Dependency Check**:
...
- For unfixable vulnerabilities, document the risk and mitigation plan
- Add to pre-push hook:
```bash
#!/bin/sh
npm audit --audit-level=high
if [ $? -ne 0 ]; then
  echo "❌ High or critical vulnerabilities found. Fix them before pushing."
  exit 1
fi
```
