...
### Default Tags
**IMPORTANT**: Use provider-level default tags for consistent resource tagging:

```hcl
provider "aws" {
  default_tags {
    tags = {
      Environment = var.environment  # e.g., "dev", "stag", "prod"
      Project     = var.project_name # e.g., "my-app"
      ManagedBy   = "Terraform"
    }
  }
}
